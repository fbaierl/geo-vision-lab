...
def extract_ontology_node(state: OntologySubGraphState) -> Dict[str, Any]:
    ...
    try:
        ...
        if not delta:
            logger.warning(
                "[ONTOLOGY_SUBGRAPH] Extractor returned None - no delta extracted"
            )
            return {
                "extracted_delta": SessionOntology(),
                "gap_entity_names": [],
        ...
        logger.info(
            f"[ONTOLOGY_SUBGRAPH] Extractor returned {len(delta.entities)} entities and {len(delta.links)} links"
        )
        ...
        for ext_ent in delta.entities:
            try:
                logger.info(
                    f"[ONTOLOGY_SUBGRAPH] Processing entity: '{ext_ent.name}' (type: {ext_ent.type})"
                )
                ...
                if ext_ent.type == "Location":
                    try:
                        ...
                        if candidates:
                            ...
                            logger.info(
                                f"[ONTOLOGY_SUBGRAPH] ✓ Geocoded '{ext_ent.name}' → lat:{best['lat']}, lon:{best['lon']}, country:{best.get('country', 'N/A')}"
                            )
                        else:
                            logger.warning(
                                f"[ONTOLOGY_SUBGRAPH] Geocoding returned no results for '{ext_ent.name}'"
                            )
                    except Exception as geo_error:
                        logger.error(
                            f"[ONTOLOGY_SUBGRAPH] Geocoding failed for '{ext_ent.name}': {geo_error}"
                        )
                ...
                entities_created += 1
                logger.info(
                    f"[ONTOLOGY_SUBGRAPH] ✓ Created entity: UUID={entity_uuid}, name='{ext_ent.name}', type={ext_ent.type}"
                )
                ...
                logger.error(
                    f"[ONTOLOGY_SUBGRAPH] Failed to process entity '{ext_ent.name}': {entity_error}"
                )
        ...
        logger.info(
            f"[ONTOLOGY_SUBGRAPH] → Links pending resolution: {len(pending_links)}"
        )
        ...
        logger.error(
            f"[ONTOLOGY_SUBGRAPH] Critical error during ontology extraction: {e}"
        )
...
def detect_gaps_node(state: OntologySubGraphState) -> Dict[str, Any]:
    ...
    if gap_names:
        logger.info(
            f"[ONTOLOGY_SUBGRAPH] Gap detection: {len(gap_names)} missing entities found"
        )
    else:
...
def route_after_gap_detection(
    state: OntologySubGraphState,
) -> Literal["extract_gap_entities", "merge_and_finalize"]:
    """
...
def extract_gap_entities_node(state: OntologySubGraphState) -> Dict[str, Any]:
    ...
    try:
        ...
        if not gap_names:
            logger.warning(
                "[ONTOLOGY_SUBGRAPH] Gap extraction called with no gap names"
            )
            return {"gap_entities_raw": []}

        logger.info(
            f"[ONTOLOGY_SUBGRAPH] Pass 2: Extracting {len(gap_names)} gap entities: {gap_names}"
        )

        extractor = get_ontology_extractor()
        gap_entities = extractor.extract_missing_entities(
            text=assistant_response, missing_names=gap_names, query=query
        )

        # Convert to raw dict format for merging
        gap_entities_raw = [
            {"name": e.name, "type": e.type, "context": e.context} for e in gap_entities
        ]

        logger.info(
            f"[ONTOLOGY_SUBGRAPH] Gap extraction complete: {len(gap_entities_raw)} entities recovered"
        )
        ...
        if unresolved:
            logger.warning(
                f"[ONTOLOGY_SUBGRAPH] Unresolved gap entities (not found in text): {unresolved}"
            )
        ...
        for gap_ent in gap_entities_raw:
            try:
                ...
                if entity_name_lower in name_to_uuid:
                    logger.warning(
                        f"[ONTOLOGY_SUBGRAPH] Gap entity '{entity_name}' already exists, skipping"
                    )
                    continue

                logger.info(
                    f"[ONTOLOGY_SUBGRAPH] Processing gap entity: '{entity_name}' (type: {gap_ent['type']})"
                )
                ...
                entity = OntologyEntity(
                    ...
                    mentions=[
                        Mention(
                            source_text=gap_ent.get("context", ""), thread_id=query_id
                        )
                    ],
                ...
                logger.info(
                    f"[ONTOLOGY_SUBGRAPH] ✓ Created gap entity: UUID={entity_uuid}, name='{entity_name}', type={gap_ent['type']}"
                )

            except Exception as entity_error:
                logger.error(
                    f"[ONTOLOGY_SUBGRAPH] Failed to process gap entity '{gap_ent.get('name', 'unknown')}': {entity_error}"
                )
                logger.exception(
                    "[ONTOLOGY_SUBGRAPH] Gap entity processing stack trace:"
                )
        ...
        for link_data in pending_links:
            try:
                ...
                link = OntologyLink(
                    ...
                    mentions=[
                        Mention(
                            source_text=link_data.get("context", ""), thread_id=query_id
                        )
                    ],
                    created_by="llm_extractor",
                )
                ...
                logger.error(
                    f"[ONTOLOGY_SUBGRAPH] Failed to process link '{link_data['source_entity_name']}' -> '{link_data['target_entity_name']}': {link_error}"
                )
                logger.exception("[ONTOLOGY_SUBGRAPH] Link processing stack trace:")

        logger.info("[ONTOLOGY_SUBGRAPH] === FINALIZATION SUMMARY ===")
        logger.info(
            f"[ONTOLOGY_SUBGRAPH] ✓ Total entities: {len(session_delta.entities)}"
        )
        logger.info(
            f"[ONTOLOGY_SUBGRAPH] ✓ Gap entities merged: {gap_entities_created}"
        )
        logger.info(f"[ONTOLOGY_SUBGRAPH] ✓ Links created: {links_created}")
        logger.info(
            f"[ONTOLOGY_SUBGRAPH] ✗ Links skipped (unresolvable): {links_skipped}"
        )
        logger.info("[ONTOLOGY_SUBGRAPH] ================================")
