...
def vector_search_node(state: RAGState) -> Dict[str, Any]:
    ...
    try:
        ...
        if not results:
            logger.info("[RAG_SUBGRAPH] No archival data found in historical intelligence database.")
            return {
...
def rerank_node(state: RAGState) -> Dict[str, Any]:
    ...
    if not settings.RAG_RERANKER_ENABLED:
        logger.info("[RAG_SUBGRAPH] Re-ranker disabled, using raw vector search results")
        # Format raw results
        results_text = "\n\n".join([doc.get("page_content", "") for doc in retrieved_docs[:settings.SEARCH_K]])
        return {
    ...
    try:
        ...
        if not reranker.is_available():
            logger.warning("[RAG_SUBGRAPH] Re-ranker model not available, falling back to vector search")
        ...
        logger.info(f"[RAG_SUBGRAPH] Re-ranking complete: {len(retrieved_docs)} → {len(reranked)} results")
...
def check_grader_result(state: RAGState) -> Literal["__end__"]:
    ...
    logger.info(f"[RAG_SUBGRAPH] Final quality: {rag_quality}, hint: {'Yes' if hint else 'No'}")
...
def get_rag_subgraph() -> StateGraph:
    ...
    logger.info(f"[RAG_SUBGRAPH] Configuration: grader={settings.RAG_GRADER_ENABLED}, reranker={settings.RAG_RERANKER_ENABLED}")
...
def run_rag_subgraph(query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"[RAG_SUBGRAPH] Subgraph complete: quality={result.get('rag_quality')}")
