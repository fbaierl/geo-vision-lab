...
class OntologyService:
    ...
    def load_ontology(self, thread_id: str) -> SessionOntology:
        ...
        """
        session = self.db.sessions.find_one({"thread_id": thread_id})
        if not session:
            return SessionOntology()
        return SessionOntology.from_array_format(session.get("ontology", {}))

    def save_ontology(self, thread_id: str, ontology: SessionOntology):
        """
    ...
    def get_neighbors(
        ...
        if hops > 1:
            for neighbor_uuid in connected_uuids:
                more = self.get_neighbors(thread_id, neighbor_uuid, hops=hops - 1)
