    ...
    def __init__(self, llm):
        ...
        if not self.is_groq:
            try:
                ...
                logger.warning(
                    "Structured output not natively supported by this LLM wrapper. Will fallback to JSON parsing."
                )
    ...
    def extract(self, text: str, query: str = "") -> Optional[OntologyDelta]:
        ...
        if self.structured_llm and not self.is_groq:
            try:
                ...
                logger.info(
                    f"[ONTOLOGY_EXTRACTOR] ✓ Structured extraction successful: {len(result.entities)} entities, {len(result.links)} links"
                )
                return result
            except Exception as e:
                logger.error(
                    f"[ONTOLOGY_EXTRACTOR] ✗ Structured extraction failed: {e}"
                )
                logger.exception(
                    "[ONTOLOGY_EXTRACTOR] Structured extraction stack trace:"
                )
        ...
        try:
            ...
            logger.debug(
                f"[ONTOLOGY_EXTRACTOR] Parsed JSON: {len(data.get('entities', []))} entities, {len(data.get('links', []))} links"
            )

            result = OntologyDelta.model_validate(data)
            logger.info(
                f"[ONTOLOGY_EXTRACTOR] ✓ Fallback extraction successful: {len(result.entities)} entities, {len(result.links)} links"
            )
            return result
            ...
            logger.error(
                f"[ONTOLOGY_EXTRACTOR] Invalid JSON content: {content[:1000] if 'content' in locals() else 'N/A'}..."
            )
            return None
        except Exception as e:
            ...
            if "data" in locals():
                logger.debug(
                    f"[ONTOLOGY_EXTRACTOR] Parsed data that failed validation: {data}"
                )
            return None

    def extract_missing_entities(
        self, text: str, missing_names: List[str], query: str = ""
    ) -> List[OntologyDeltaEntity]:
        """
        ...
        if not text.strip() or not missing_names:
            logger.warning(
                "[ONTOLOGY_EXTRACTOR] Empty text or missing names for gap extraction"
            )
            return []

        logger.info(
            f"[ONTOLOGY_EXTRACTOR] Starting gap extraction for {len(missing_names)} entities: {missing_names}"
        )
        ...
        try:
            ...
            logger.debug(
                f"[ONTOLOGY_EXTRACTOR] Parsed gap JSON: {len(entities_data)} entities"
            )

            # Validate and return entities
            result = OntologyDelta.model_validate(data)
            logger.info(
                f"[ONTOLOGY_EXTRACTOR] ✓ Gap extraction successful: {len(result.entities)} entities recovered"
            )

            # Log which missing entities were found
            found_names = [e.name for e in result.entities]
            not_found = set(n.lower() for n in missing_names) - set(
                n.lower() for n in found_names
            )
            if not_found:
                logger.warning(
                    f"[ONTOLOGY_EXTRACTOR] Could not find these entities in text: {not_found}"
                )
            ...
            logger.error(
                f"[ONTOLOGY_EXTRACTOR] ✗ Gap extraction JSON parsing failed: {json_err}"
            )
            logger.exception("[ONTOLOGY_EXTRACTOR] JSON decode stack trace:")
            logger.error(
                f"[ONTOLOGY_EXTRACTOR] Invalid JSON content: {content[:1000] if 'content' in locals() else 'N/A'}..."
...
def get_ontology_extractor() -> OntologyExtractorService:
    ...
    return container._get_or_create(
        get_ontology_extractor, lambda: OntologyExtractorService(llm=get_llm())
    )
