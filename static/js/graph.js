...
export function renderGraph(ontology, container) {
    if (!container) return;
    ...
    if (ontology.entities) {
        for (const [id, ent] of Object.entries(ontology.entities)) {
            ...
            tooltipHtml.innerHTML = `<b>${escapeHtml(ent.name)}</b><br><i>${escapeHtml(ent.type)}</i><br><br>${propertiesStr}`;
    ...
    if (ontology.links) {
        for (const [id, link] of Object.entries(ontology.links)) {
            ...
            if (ontology.entities[sourceId] && ontology.entities[targetId]) {
                edges.push({
                    id: id,
                    from: sourceId,
                    to: targetId,
                ...
                if (!ontology.entities[sourceId]) {
                    console.warn(`Edge ${id}: Source entity ${sourceId} not found`);
                }
                if (!ontology.entities[targetId]) {
                    console.warn(`Edge ${id}: Target entity ${targetId} not found`);
                }
            }
        }
    }
