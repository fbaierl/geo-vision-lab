...
def test_real_db_ingestion_and_search(postgres_container):
    """
    Integration test:
    ...
    # Mock settings so the app points to our ephemeral DB
    # We set a very aggressive chunk size to force multiple chunks for testing!
    ...
    # We also need to supply string values for the LLM so pydantic doesn't fail
    mock_settings.LLM_MODEL_NAME = "qwen3.5:9b"
    mock_settings.OLLAMA_BASE_URL = "http://localhost:11434"
    
    with patch("app.core.config.settings", mock_settings):
        # We also need to patch get_settings() in case any module calls it directly
