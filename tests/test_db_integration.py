...
def test_real_db_ingestion_and_search(mongodb_container, monkeypatch):
    ...
    user = mongodb_container.MONGO_INITDB_ROOT_USERNAME
    password = mongodb_container.MONGO_INITDB_ROOT_PASSWORD
    dbname = "geovision_test"

    # Use monkeypatch to modify the global settings singleton correctly.
    from app.core.config import settings

    monkeypatch.setattr(settings, "MONGODB_USER", user)
    monkeypatch.setattr(settings, "MONGODB_PASSWORD", password)
    monkeypatch.setattr(settings, "MONGODB_SERVER", host)
    monkeypatch.setattr(settings, "MONGODB_PORT", str(port))
    ...
    monkeypatch.setattr(settings, "DATABASE_URL", f"mongodb://{user}:{password}@{host}:{port}/{dbname}?authSource=admin")
