...
class TestOntologyGapResolution:
    ...
    def test_gap_detection_identifies_missing_entities(self):
        ...
        mock_delta = OntologyDelta(
            entities=[
                OntologyDeltaEntity(
                    name="United States",
                    type="Location",
                    context="The Allies included United States",
                ),
                OntologyDeltaEntity(
                    name="United Kingdom",
                    type="Location",
                    context="United Kingdom was part of Allies",
                ),
                OntologyDeltaEntity(
                    name="Nazi Germany",
                    type="Location",
                    context="Axis powers included Nazi Germany",
                ),
            ],
            links=[
                OntologyDeltaLink(
                    source_entity_name="United States",
                    target_entity_name="Allies",
                    relationship_type="PART_OF",
                    context="The Allies included United States",
                ),
                OntologyDeltaLink(
                    source_entity_name="Nazi Germany",
                    target_entity_name="Axis powers",
                    relationship_type="PART_OF",
                    context="Axis powers included Nazi Germany",
                ),
            ],
        ...
        with patch(
            "app.agents.ontology_subgraph.get_ontology_extractor"
    ...
    def test_gap_detection_no_gaps(self):
        ...
        mock_delta = OntologyDelta(
            entities=[
                OntologyDeltaEntity(
                    name="John", type="Person", context="John works at Microsoft"
                ),
                OntologyDeltaEntity(
                    name="Microsoft", type="Organization", context="works at Microsoft"
                ),
                OntologyDeltaEntity(
                    name="Seattle", type="Location", context="Microsoft in Seattle"
                ),
            ],
            links=[
                OntologyDeltaLink(
    ...
    def test_detect_gaps_routes_to_extraction(self):
        ...
        state_with_gaps = {"gap_entity_names": ["Allies", "Axis powers"]}

        result = route_after_gap_detection(state_with_gaps)
        assert result == "extract_gap_entities"

        state_without_gaps = {"gap_entity_names": []}
    ...
    def test_extract_gap_entities_calls_extractor(self):
        ...
        mock_gap_entities = [
            OntologyDeltaEntity(
                name="Allies",
                type="Organization",
                context="The Allies and Axis powers fought in WW2",
            ),
            OntologyDeltaEntity(
                name="Axis powers",
                type="Organization",
            ...
            "gap_entities_raw": [
                {
                    "name": "Allies",
                    "type": "Organization",
                    "context": "The Allies fought",
                },
                {
                    "name": "Axis powers",
                    "type": "Organization",
                    "context": "Axis powers fought",
                },
            ],
            "pending_links": [
                {
    ...
    def test_merge_and_finalize_skips_unresolvable_links(self):
        ...
        assert (
            len(final_delta.links) == 0
        )  # Actually 0 because "United States" doesn't exist either
