...
class TestSessionsAPI:
    ...
    def test_create_session(self, client_with_mongo_mock, override_mongo):
        ...
        response = client_with_mongo_mock.post(
            "/api/sessions", json={"title": "Test Session"}
        )
    ...
    def test_update_session_title(self, client_with_mongo_mock, override_mongo):
        ...
        # Create a session
        create_response = client_with_mongo_mock.post(
            "/api/sessions", json={"title": "Old Title"}
        )
        thread_id = create_response.json()["thread_id"]

        # Update title
        update_response = client_with_mongo_mock.put(
            f"/api/sessions/{thread_id}", json={"title": "New Title"}
        )
    ...
    def test_save_session_update_existing(self, client_with_mongo_mock, override_mongo):
        ...
        create_response = client_with_mongo_mock.post(
            "/api/sessions", json={"title": "Initial"}
        )
        thread_id = create_response.json()["thread_id"]
    ...
    def test_sessions_sorted_by_updated_at(
        self, client_with_mongo_mock, override_mongo
    ):
        """Test that sessions are sorted by updated_at (most recent first)."""
        # Create two sessions
        response1 = client_with_mongo_mock.post(
            "/api/sessions", json={"title": "First"}
        )
