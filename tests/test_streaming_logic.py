    ...
    async def mock_astream_events(*args, **kwargs):
        ...
        for i, chunk_text in enumerate(chunks):
            yield {
                ...
                "name": "ChatGroq",
                "tags": ["seq:step:1"],
                "data": {"chunk": AIMessageChunk(content=chunk_text)},
            }
        ...
        yield {
            ...
            "name": "ChatGroq",
            "data": {"output": AIMessageChunk(content="".join(chunks))},
            "tags": ["seq:step:1"],
        }
    ...
    try:
        ...
        async with asyncio.timeout(2.0):  # 2 seconds is plenty for this mock
            async for event in process_query_stream(
                user_query, thread_id="test", graph_override=mock_graph
            ):
                events.append(event)
    except asyncio.TimeoutError:
        pytest.fail(
            "process_query_stream hung (likely infinite loop in buffer processing)"
        )
